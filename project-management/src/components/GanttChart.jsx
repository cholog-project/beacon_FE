    ...
    return (
        <div className="gantt-container" ref={chartRef}>
            ...
            {tasks.map((task) => {
                ...
                const startDay = Math.max(Math.round((startDate - startMonth) / (1000 * 60 * 60 * 24)), 0);
                ...
                return (
                    <div key={task.id} className="gantt-task-row">
                        <div
                            ...
                            style={{
                                marginLeft: `${startDay * dayWidth * 0.98}px`,  // 시작 날짜에 맞게 위치 조정
                                width: `${taskDuration * dayWidth}px`,   // 작업 기간에 맞게 길이 조정
                            }}
                        >
                            {/* Task 이름을 검정색으로 표시 */}
